...
normalisation, and progressive growing~\cite{karras2018progressive}, was required before GAN outputs reached
...
is separated from its style, and synthesis continues by progressively adding detail at
...
synthetic face collections are visually difficult to distinguish from stock photography. Dataset~C,
...
stride~2 introduces a checkerboard pattern~\cite{odena2016deconvolution} in the spatial domain that appears as spectral
peaks at Nyquist/2 in the FFT magnitude spectrum. These peaks are absent from natural
photographs, whose spatial statistics follow an approximate $1/f$ decay~\cite{ruderman1994statistics}. Detectors that
operated on the frequency spectrum of early GAN outputs could reliably identify these peaks
as evidence of generation. The artefacts were a consequence of the discriminator
...
network conditions generation on an external signal, typically a CLIP~\cite{radford2021clip} text embedding,
enabling text-to-image generation. The resulting Stable Diffusion models, beginning with
version~1.5 and progressing through SD~2.1, SDXL~\cite{podell2023sdxl}, and SD~3~\cite{esser2024sd3}, produce high-resolution
...
outputs, while Dataset~B uses outputs from five diffusion-based generators: SD~2.1,
...
image across hundreds of denoising steps, with each step adding small corrections
...
spectral signatures face a harder challenge when the target is a diffusion model rather than
a GAN.
...
generators available at the time, and each iteration of generators aims to remove or weaken
those artefacts. Early GAN architectures (2014--2018) produced outputs with strong
checkerboard artefacts from transposed convolution upsampling~\cite{odena2016deconvolution}, visible blurring at
...
of steps. This change made spectral detectors unreliable as a class.

Before CNN-based classifiers, AI image detection relied on hand-crafted features drawn
from the signal processing and digital forensics literature. To capture the unique statistical 
fingerprint left behind by AI synthesis, researchers used JPEG quantization statistics, 
co-occurrence matrices of noise residuals, and Spatial Rich Model features~\cite{fridrich2012rich}. 
These methods achieved reasonable performance on early GAN architectures whose upsampling operations introduced
...
and less consistent across architectures, and performance degraded. The move to CNN feature 
extraction was driven by the need to adapt to diverse, generator-specific fingerprints instead 
of relying on a fixed dictionary of features.
...
different GAN architectures, from a range of generator families.

The main finding of Wang et al.\ is that training on one generator produces near-perfect
in-distribution performance, but substantially poorer results on other generators.
Even with augmentation, cross-generator F1 drops to near-random for architectures whose
spectral footprint differs significantly from the training distribution. This discovery 
that CNN detectors learn generator-specific shortcuts rather than universal artefacts 
is the central challenge addressed in this thesis. The work of Wang et al.\
was conducted entirely within the GAN paradigm on a single real-image source, while this thesis
extends it to a cross-paradigm setting with three independent real-image sources and a
controlled four-stage training process.
...
gradients. ResNet-50 (${\approx}$25.6M parameters) and ResNet-18 (${\approx}$11.7M parameters) are used in
...
spectral content follows a natural $1/f$ decay~\cite{ruderman1994statistics}. Detectors that operate in the frequency
domain or use spectral features as input can exploit these patterns directly~\cite{zhang2019detecting,frank2020leveraging}.
...
rather than the raw RGB input, the task of identifying spectral anomalies is
...
the CLIP-based universal detector was evaluated. The inclusion of diffusion models makes
...
and multi-real-source coverage. Dataset~A~\cite{datasetA_kaggle} is the ``AI vs.\
Human-Generated Images'' Kaggle dataset: approximately 80,000 balanced images with Stable
Diffusion~v1.5~\cite{rombach2022} as the AI source and Shutterstock stock photographs as the real source.
Dataset~B is the Defactify Image Dataset~\cite{roy2025defactify}, available on HuggingFace: 96,000 images from
five diffusion-based generators (SD~2.1, SDXL, SD~3, DALL-E~3, MidJourney~v6) paired
with MS-COCO natural photographs at a ratio of 1:5 real-to-AI imbalance. Metadata identifies
which generator produced each AI image, enabling per-generator evaluation. Dataset~C~\cite{datasetC_kaggle}
...
against a specific real-image source. This thesis groups them together for a different purpose: 
to evaluate how well a model trained on both real and AI classes of a 
dataset performs on another dataset's distribution. The answer depends not only on the
generator but also on the real-image source, a dependency that the cross-dataset matrix
makes obvious.

The choice of real-image source is an essential matter for this thesis. Detectors used
...
when the real-image shift is equally responsible. The matrix design here decouples the
...
\begin{table}[htb]
  ...
  \begin{tabular}{llll}
    ...
    Real-source variation & No                   & No                    & Yes \\
    \hline
  \end{tabular}}
\end{table}
